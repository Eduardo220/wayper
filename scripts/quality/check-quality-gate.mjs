...
export function synthesizeQuality({ lint, size, architecture, router, evidence, validation, graph, contextEconomy, completion, feedback, dispatch, crossRepo, memory, diff }) {
  const checks = { size, architecture, router, evidence, validation, ...(graph ? { graph } : {}),
    ...(contextEconomy ? { contextEconomy } : {}), ...(completion ? { completion } : {}), ...(feedback ? { feedback } : {}), ...(dispatch ? { dispatch } : {}),
    ...(crossRepo ? { crossRepo } : {}), ...(memory ? { memory } : {}), diff };
  const blocking = [];
  const toolFailures = [];
...
export function formatQuality(result, { details = false, json = false } = {}) {
  ...
  const lines = [
    ...
    ...(result.checks.graph ? [`graph: ${result.checks.graph.status === 'pass' ? 'freshness and cache contracts valid' : result.checks.graph.status}`] : []),
    ...
    ...(result.checks.crossRepo ? [`cross-repo: ${result.checks.crossRepo.status === 'pass' ? 'project coordination contracts valid' : result.checks.crossRepo.status}`] : []),
    ...(result.checks.memory ? [`memory: ${result.checks.memory.status === 'pass' ? 'promotion and retrieval contracts valid' : result.checks.memory.status}`] : []),
...
async function executeQualityGate() {
  ...
  const [lintRun, sizeRun, architectureRun, routerRun, evidenceRun, validationRun, graphRun, contextEconomyRun,
    ...
    run(process.execPath, ['--test', path.join(ROOT, 'scripts/quality/check-graph-context.test.mjs')]),
    run(process.execPath, ['--test', path.join(ROOT, 'scripts/quality/check-context-economy.test.mjs'),
      path.join(ROOT, 'scripts/quality/check-context-adversarial.test.mjs')]),
  ...
  // Lease-expiry subprocess tests need a scheduler window smaller than their 500 ms TTL.
  const dispatchRun = await run(process.execPath, ['--test', ...['check-dispatch', 'check-dispatch-spawn', 'check-dispatch-circuit',
    'check-ownership', 'check-ownership-concurrency'].map(name => path.join(ROOT, `scripts/quality/${name}.test.mjs`))]);
  const [crossRepoRun, memoryRun] = await Promise.all([
    run(process.execPath, ['--test', path.join(ROOT, 'scripts/quality/check-cross-repo.test.mjs')]),
    run(process.execPath, ['--test', path.join(ROOT, 'scripts/quality/check-project-memory.test.mjs')]),
  ]);
  ...
  return synthesizeQuality({
    ...
    graph: classifyCommand(graphRun),
    ...
    crossRepo: classifyCommand(crossRepoRun),
    memory: classifyCommand(memoryRun),
    diff,
  });
}
